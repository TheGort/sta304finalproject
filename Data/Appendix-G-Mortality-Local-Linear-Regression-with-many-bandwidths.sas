*t1m.sas - local linear regressions;
...
%end;
data external_comb;
   set  %do i = 1 %to &bins.;external_all_&i.%end;;
run;
%do i = 1 %to &bins.;
    %ll(internal_all,&i.);
%end;
data internal_comb;
   set  %do i = 1 %to &bins.;internal_all_&i.%end;;
run;
*Clean up;
%do i = 1 %to &bins.;
 proc datasets;
    delete %do i = 1 %to &bins.;internal_all_&i. external_all_&i%end;
 run;
%end;
%mend;
*CANT JUST UP THE NUMBER OF BINS HAVE TO ADJUST THE DENOMINATOR;
%junk(40);
*%ll(internal_all);
data all_fit;
   merge external_comb internal_comb;
   by h;
run;


PROC EXPORT DATA= all_fit
            OUTFILE= "C:\Documents and Settings\RDC3\Desktop\Dobkin\Alcohol 21 for DC March 2007\Appendix H Mortality Local Linear Regression with many bandwidths.csv" 
            DBMS=csv REPLACE;
RUN;
